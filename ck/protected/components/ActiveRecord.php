...
class ActiveRecord extends CActiveRecord
{
    ...
    {
		if (!am()->checkAccess('BackOffice', user()->id))
		{
			$alias = $this->getTableAlias(false, false);
			return array(
				'condition'=>$alias.'.user_id='.user()->id,
			);
		}
		else
			return array();
    ...
    */
	...
	public function scopes()
	{
		...
		return array(
			'my'=>array(
				'condition'=>((am()->checkAccess('BackOffice', user()->id)) ? '' : $alias.'.user_id='.user()->id),
			),
		);
	}
	...
	public function scopesWoAlias()
	{
		$scopes = $this->scopes();
		foreach ($scopes as $k=>$scope)
			$scopes[$k]['condition'] = str_replace('t.', '', $scopes[$k]['condition']);
		return $scopes;
	}
